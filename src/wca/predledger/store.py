...
def ensure_schema(db: str = _DEFAULT_DB) -> None:
    """Create tables, indexes and views (idempotent, safe to re-run)."""
    with _connect_write(db) as conn:
...
def upsert_predictions(rows: Iterable[Dict[str, Any]], db: str = _DEFAULT_DB) -> int:
    """Insert-or-replace prediction rows; return the count written.
...
def link_bet(pred_id: str, bet_id: int, db: str = _DEFAULT_DB) -> None:
...
def get_prediction(pred_id: str, db: str = _DEFAULT_DB) -> Optional[sqlite3.Row]:
    with _connect_read(db) as conn:
        return conn.execute(
            "SELECT * FROM predictions WHERE prediction_id=?", (pred_id,)
        ).fetchone()


def all_predictions(db: str = _DEFAULT_DB) -> List[sqlite3.Row]:
    with _connect_read(db) as conn:
        return conn.execute(
...
def open_predictions(
    db: str = _DEFAULT_DB, market: Optional[str] = None
) -> List[sqlite3.Row]:
...
def model_book(db: str = _DEFAULT_DB) -> List[sqlite3.Row]:
    with _connect_read(db) as conn:
        return conn.execute("SELECT * FROM v_model_book").fetchall()


def realized_book(db: str = _DEFAULT_DB) -> List[sqlite3.Row]:
    with _connect_read(db) as conn:
