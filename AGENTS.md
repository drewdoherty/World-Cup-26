...
- **Generated site feeds** (`site/*.json`) — build output of the card/feed/sync
  jobs; untracked. They are regenerated by the mini (`deploy/publish_site.sh`) and
  rsynced to the MacBook (`deploy/macbook/pull_feeds.sh`) — git is not their
  transport. Do not re-add them to `git add` lists. (`site/microstructure/*.json`
  is frozen one-off recon output, still tracked for now — see
