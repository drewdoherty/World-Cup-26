...
- **The microstructure scripts' own disclaimer** describes the gap exactly (`scripts/microstructure/liquidity.py:6-8`): "NO order-book depth, NO matched volume, NO queue position … left as a FRAMEWORK" — describing data the CLOB endpoints DO expose and the pipeline never retains.
