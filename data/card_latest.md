<!-- generated: 2026-07-09T16:00:18 -->
*World Cup Alpha — trade card* (1 staked picks, hit-prob ranked)
_order: model-prob bucket (ML ≥50% > MID 25–50% > LS <25%), further-out first, EV tiebreak — wca.selection_

*1. [ML·FAV] Norway vs England* — England — back *52.1%* impl via *betfair*
    model 53.3% / mkt 51.9% (+1.2pp)  EV *+2.2%* ✅+EV  [elo 46% dc 58%]
    stake: gbp £19.91  further-out (53h) — thin/soft market

*— CUT (excluded from staking, 3) —*
  x [LS] Spain vs Belgium — Belgium — back 16.9% impl (model 18.4%, EV +8.7% ✅+EV): below 20% hit-probability floor (model 18.4%) — too unlikely to return PnL even at +8.7% EV
  x [LS] France vs Morocco — Morocco — back 14.3% impl (model 16.4%, EV +7.4% ✅+EV): below 20% hit-probability floor (model 16.4%) — too unlikely to return PnL even at +7.4% EV
  x [LS] Argentina vs Switzerland — Switzerland — back 16.1% impl (model 17.0%, EV +5.3% ✅+EV): below 20% hit-probability floor (model 17.0%) — too unlikely to return PnL even at +5.3% EV

*— WATCH (near-threshold, below the +2% EV floor — NOT staked, 2) —*
  ~ [ML] Argentina vs Switzerland — Argentina — back 57.1% impl via betfair (model 57.7% / mkt 57.0%, EV +1.0% ✅+EV)
  ~ [MID] France vs Morocco — Draw — back 25.0% impl via betfair (model 25.7% / mkt 24.6%, EV +1.5% ✅+EV)

*Venue split (gbp):* betfair £19.91

*— REFERENCE, NOT SIZED (model prices only) —*
*World Cup Alpha — scorelines* (4 fixtures)

*France vs Morocco*
    xG: 1.53-0.72
    1-0  16.8%  back at impl <= 16.5%
    2-0  12.5%  back at impl <= 12.3%
    1-1  11.7%  back at impl <= 11.5%
    0-0  10.5%  back at impl <= 10.3%
    2-1  9.7%  back at impl <= 9.6%
    0-1  6.6%  back at impl <= 6.4%
    O/U 2.5: over 39.1% / under 60.9%   BTTS 41.0%

*Spain vs Belgium*
    xG: 1.92-1.06
    1-1  11.2%  back at impl <= 11.0%
    2-1  10.6%  back at impl <= 10.4%
    1-0  9.7%  back at impl <= 9.5%
    2-0  9.4%  back at impl <= 9.2%
    3-1  6.5%  back at impl <= 6.4%
    3-0  5.8%  back at impl <= 5.7%
    O/U 2.5: over 57.0% / under 43.0%   BTTS 57.0%

*Norway vs England*
    xG: 0.95-1.71
    1-1  12.1%  back at impl <= 11.9%
    0-1  10.7%  back at impl <= 10.5%
    0-2  10.1%  back at impl <= 9.9%
    1-2  8.9%  back at impl <= 8.8%
    0-0  7.8%  back at impl <= 7.7%
    1-0  6.6%  back at impl <= 6.5%
    O/U 2.5: over 49.4% / under 50.6%   BTTS 50.3%

*Argentina vs Switzerland*
    xG: 1.82-0.91
    1-1  12.1%  back at impl <= 11.8%
    1-0  11.2%  back at impl <= 11.0%
    2-0  10.6%  back at impl <= 10.4%
    2-1  9.9%  back at impl <= 9.7%
    0-0  7.4%  back at impl <= 7.3%
    3-0  6.4%  back at impl <= 6.2%
    O/U 2.5: over 51.1% / under 48.9%   BTTS 51.5%